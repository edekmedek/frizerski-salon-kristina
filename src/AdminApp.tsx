...
import { isValidAdminPin, normalizeAdminPin } from './lib/adminPin'
import { supabase } from './lib/supabase'
import './Portal.css'
import './AdminPortal.css'

type View = 'pregled' | 'klijenti' | 'termini' | 'cjenik' | 'zahtjevi' | 'poruke' | 'arhiva' | 'postavke' | 'arhiva-termina'
...
function AdminApp({ onLogout }: { onLogout: () => void }) {
  ...
  useEffect(() => {
    async function loadSupabaseClients() {
      ...
      const [{ data: clients, error }, { data: statuses }, { data: appointments }, { data: services }, { data: categories }] = await Promise.all([
        ...
        supabase.from('appointments').select('id,client_id,starts_at,service,status,notes,created_at,updated_at,service_id,service_name_snapshot,service_price_snapshot,service_duration_snapshot,no_charge'),
  ...
  async function saveAppointment(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (supabase) {
      ...
      const values = {
        ...
        no_charge: appointmentForm.noCharge === true,
      }
  ...
  return <div className="app-shell">
    ...
    {adminPinDialog&&<Modal title={adminPinDialog==='unlock'?'Otključaj arhivu termina':adminPinDialog==='change'?'Promijeni administratorski PIN':'Postavi administratorski PIN'} onClose={closeAdminPinDialog}><form className="admin-pin-form" onSubmit={submitAdminPin}>{adminPinDialog==='change'&&<label>Trenutačni PIN<input type="password" inputMode="numeric" autoComplete="current-password" placeholder="••••" minLength={4} maxLength={4} pattern="[0-9]{4}" value={currentAdminPin} onChange={event=>setCurrentAdminPin(normalizeAdminPin(event.target.value))}/></label>}<label>{adminPinDialog==='unlock'?'Administratorski PIN':'Novi PIN'}<input autoFocus type="password" inputMode="numeric" autoComplete="off" placeholder="••••" minLength={4} maxLength={4} pattern="[0-9]{4}" value={adminPin} onChange={event=>setAdminPin(normalizeAdminPin(event.target.value))}/><small>Točno 4 znamenke.</small></label>{adminPinDialog!=='unlock'&&<label>Ponovite novi PIN<input type="password" inputMode="numeric" autoComplete="off" placeholder="••••" minLength={4} maxLength={4} pattern="[0-9]{4}" value={adminPinConfirm} onChange={event=>setAdminPinConfirm(normalizeAdminPin(event.target.value))}/></label>}{adminPinError&&<p className="form-message" role="alert">{adminPinError}</p>}<div className="form-actions"><button className="secondary" type="button" onClick={closeAdminPinDialog}>Odustani</button><button className="primary" type="submit" disabled={adminPinBusy||!isValidAdminPin(adminPin)||(adminPinDialog!=='unlock'&&!isValidAdminPin(adminPinConfirm))||(adminPinDialog==='change'&&!isValidAdminPin(currentAdminPin))}>{adminPinBusy?'Provjera…':adminPinDialog==='unlock'?'Otključaj':'Spremi PIN'}</button></div></form></Modal>}
