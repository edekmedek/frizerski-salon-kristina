...
function AdminApp({ onLogout }: { onLogout: () => void }) {
  ...
  return <div className="app-shell">
    ...
    <main><header><div><p className="eyebrow">Salon Kristina</p><h1>{title}</h1></div><div className="header-actions"><span className="status-dot" /> Lokalno spremljeno</div></header>
      ...
      {view === 'cjenik' && <section className="panel"><div className="panel-head"><div><p className="eyebrow">USLUGE I CIJENE</p><h2>Cjenik</h2></div><div className="toolbar"><button className="secondary" onClick={() => setCategoryForm({id:'',name:'',isActive:true,displayOrder:categoryCatalog.length+1})}>+ Nova kategorija</button><button className="primary" disabled={!categoryCatalog.length} onClick={() => { const category=categoryCatalog[0]; setServiceForm({id:'',categoryId:category.id,categoryName:category.name,name:'',price:0,isActive:true,isBookable:true,displayOrder:serviceCatalog.length+1}) }}>+ Nova stavka</button></div></div><h3>Kategorije</h3><div className="table-wrap price-category-table"><table><thead><tr><th>Red.</th><th>Naziv kategorije</th><th>Aktivna</th><th /></tr></thead><tbody>{orderedCategories(categoryCatalog).map(category=><tr key={category.id}><td>{category.displayOrder}</td><td>{category.name}</td><td>{category.isActive?'Da':'Ne'}</td><td><button className="link" onClick={()=>setCategoryForm(category)}>Uredi</button></td></tr>)}</tbody></table></div><h3>Usluge</h3><div className="table-wrap"><table className="price-table"><thead><tr><th>Red.</th><th>Kategorija</th><th>Naziv</th><th>Cijena</th><th>Trajanje</th><th>Aktivna</th><th>Za termin</th><th /></tr></thead><tbody>{serviceCatalog.map(item=><tr key={item.id}><td>{item.displayOrder}</td><td>{item.categoryName}</td><td>{item.name}</td><td>{item.price.toLocaleString('hr-HR',{style:'currency',currency:'EUR'})}</td><td>{item.durationMinutes ? `${item.durationMinutes} min` : 'Nije uneseno'}</td><td>{item.isActive?'Da':'Ne'}</td><td>{item.isBookable?'Da':'Dodatak'}</td><td><button className="link" onClick={()=>setServiceForm(item)}>Uredi</button></td></tr>)}</tbody></table></div></section>}
