...
import { createEmptyAdminPinFields, isValidAdminPin, isValidCurrentAdminPin, normalizeAdminPin, normalizeCurrentAdminPin } from './lib/adminPin'
...
function AdminApp({ onLogout }: { onLogout: () => void }) {
  ...
  return <div className="app-shell">
    ...
    {adminPinDialog&&<Modal title={adminPinDialog==='unlock'?'Otključaj arhivu termina':adminPinDialog==='change'?'Promijeni administratorski PIN':'Postavi administratorski PIN'} onClose={closeAdminPinDialog}><form className="admin-pin-form" autoComplete="off" onSubmit={submitAdminPin}>{adminPinDialog==='change'&&<label>Trenutačni PIN<input type="password" name="calendar-admin-current-pin-transition" inputMode="numeric" autoComplete="off" data-1p-ignore="true" data-lpignore="true" minLength={4} maxLength={6} pattern="[0-9]{4,6}" value={currentAdminPin} onChange={event=>setCurrentAdminPin(normalizeCurrentAdminPin(event.target.value))}/></label>}<label>{adminPinDialog==='unlock'?'Administratorski PIN':'Novi PIN'}<input autoFocus type="password" name={adminPinDialog==='unlock'?'calendar-archive-pin-entry':'calendar-admin-new-pin'} inputMode="numeric" autoComplete="off" data-1p-ignore="true" data-lpignore="true" minLength={4} maxLength={4} pattern="[0-9]{4}" value={adminPin} onChange={event=>setAdminPin(normalizeAdminPin(event.target.value))}/><small>Točno 4 znamenke.</small></label>{adminPinDialog!=='unlock'&&<label>Ponovite novi PIN<input type="password" name="calendar-admin-new-pin-confirmation" inputMode="numeric" autoComplete="off" data-1p-ignore="true" data-lpignore="true" minLength={4} maxLength={4} pattern="[0-9]{4}" value={adminPinConfirm} onChange={event=>setAdminPinConfirm(normalizeAdminPin(event.target.value))}/></label>}{adminPinError&&<p className="form-message" role="alert">{adminPinError}</p>}<div className="form-actions"><button className="secondary" type="button" onClick={closeAdminPinDialog}>Odustani</button><button className="primary" type="submit" disabled={adminPinBusy||!isValidAdminPin(adminPin)||(adminPinDialog!=='unlock'&&!isValidAdminPin(adminPinConfirm))||(adminPinDialog==='change'&&!isValidCurrentAdminPin(currentAdminPin))}>{adminPinBusy?'Provjera…':adminPinDialog==='unlock'?'Otključaj':'Spremi PIN'}</button></div></form></Modal>}
