...
export function ClientPortal({ clientId, onLogout }: { clientId: string; onLogout: () => void }) {
  ...
  return <div className="client-portal">
    ...
    <nav className="client-nav"><button className={section==='home'?'active':''} onClick={()=>setSection('home')}>Pregled</button><button className={section==='appointments'?'active':''} onClick={()=>setSection('appointments')}>Termini</button><button className={section==='prices'?'active':''} onClick={()=>setSection('prices')}>Cjenik</button><button className={section==='messages'?'active':''} onClick={()=>setSection('messages')}>Poruke</button><button className={section==='photos'?'active':''} onClick={()=>setSection('photos')}>Fotografije</button></nav>
    <main className="client-content">
      ...
      {section==='prices'&&<section className="client-card-section"><h2>Cjenik</h2><div className="client-price-groups">{priceCategories.map(category=><section key={category}><h3>{category}</h3><div className="client-price-list">{priceList.filter(item=>item.categoryName===category).map(item=><div key={item.name}><span>{item.name}</span><strong>{item.price.toLocaleString('hr-HR',{style:'currency',currency:'EUR'})}</strong></div>)}</div></section>)}</div><p className="privacy-note">U cijene su uključeni svi porezi.</p></section>}
