...
interface PendingRequestSubmission { preferredDates: string[]; dayPeriod: string; message: string }
...
export function ClientPortal({ clientId, onLogout }: { clientId: string; onLogout: () => void }) {
  ...
  const [pendingRequestSubmission, setPendingRequestSubmission] = useState<PendingRequestSubmission | null>(null)
  ...
  function reviewRequest(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault()
    ...
    setPendingRequestSubmission({
      preferredDates,
      dayPeriod: String(form.get('dayPeriod')),
      message: String(form.get('message') || '').trim(),
    })
  }

  async function saveRequest() {
    if (!supabase || !pendingRequestSubmission || requestSubmitting) return
    ...
    const { data, error } = await supabase.rpc('client_submit_request', {
      ...
      requested_dates: pendingRequestSubmission.preferredDates,
      requested_day_period: pendingRequestSubmission.dayPeriod,
      request_message: pendingRequestSubmission.message,
    ...
    setRequestCategory('')
    setPendingRequestSubmission(null)
    setRequestSubmitting(false)
  ...
  return <div className="client-portal">
    ...
    <main className="client-content">
      ...
      {section==='request'&&<section className="client-card-section client-request-screen"><h2>Želja za termin</h2><p className="important-note">Ovo nije rezervacija. Kristina će pregledati vašu želju i potvrditi termin.</p><form onSubmit={reviewRequest}>
        <div className="client-treatment-summary"><strong>Odabrane usluge</strong>{requestServiceIds.length?<div className="client-treatment-chips">{bookableServices.filter(item=>item.id&&requestServiceIds.includes(item.id)).map(item=><span key={item.id}>{item.name}<button type="button" aria-label={`Ukloni ${item.name}`} onClick={()=>setRequestServiceIds(current=>current.filter(id=>id!==item.id))}>×</button></span>)}</div>:<p>Usluga nije odabrana</p>}</div>
        {!requestCategory?<div className="client-service-categories"><strong>Dodajte uslugu po kategoriji</strong>{requestCategories.map(category=><button type="button" key={category} onClick={()=>setRequestCategory(category)}><span>{category}</span><b aria-hidden="true">›</b></button>)}</div>:<div className="client-service-options"><button className="link" type="button" onClick={()=>setRequestCategory('')}>← Sve kategorije</button><strong>{requestCategory}</strong>{bookableServices.filter(item=>item.categoryName===requestCategory&&!requestServiceIds.includes(item.id as string)).map(item=><button type="button" key={item.id} onClick={()=>{if(item.id)setRequestServiceIds(current=>current.includes(item.id as string)?current:[...current,item.id as string]);setRequestCategory('')}}><span><strong>{item.name}</strong><small>{item.price.toLocaleString('hr-HR',{style:'currency',currency:'EUR'})}{item.durationMinutes?` · ${item.durationMinutes} min`:''}</small></span><b aria-hidden="true">+</b></button>)}</div>}
        ...
        <label>Dio dana<select name="dayPeriod" defaultValue="any"><option value="morning">Prijepodne</option><option value="afternoon">Poslijepodne</option><option value="any">Svejedno</option></select></label>
        <label>Dodatne želje<textarea name="message" rows={4}/></label>
        <div className="client-request-submit-bar"><button className="primary" type="submit">Pošalji Kristini</button><button className="secondary" type="button" onClick={()=>{setRequestCategory('');setSection('home')}}>Odustani</button></div>
      </form></section>}
    ...
    {pendingRequestSubmission&&<div className="portal-modal-backdrop"><section className="portal-modal request-confirmation" role="dialog" aria-modal="true" aria-labelledby="request-confirmation-title"><h2 id="request-confirmation-title">Poslati želju Kristini?</h2><dl><div><dt>Datum</dt><dd>{pendingRequestSubmission.preferredDates.map(formatDate).join(', ')}</dd></div><div><dt>Usluge</dt><dd>{bookableServices.filter(item=>item.id&&requestServiceIds.includes(item.id)).map(item=>item.name).join(' + ')||'Usluga nije odabrana'}</dd></div>{pendingRequestSubmission.message&&<div><dt>Napomena</dt><dd>{pendingRequestSubmission.message}</dd></div>}</dl><div className="portal-modal-actions"><button className="primary" disabled={requestSubmitting} type="button" onClick={()=>void saveRequest()}>{requestSubmitting?'Slanje…':'Pošalji'}</button><button className="secondary" disabled={requestSubmitting} type="button" onClick={()=>setPendingRequestSubmission(null)}>Odustani</button></div></section></div>}
